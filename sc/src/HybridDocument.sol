...
contract HybridDocument is ERC721, ERC721URIStorage, Ownable {
    ...
    ) ERC721("Velipe Document", "VDOC") Ownable(msg.sender) {
        issuerRegistry = IIssuerRegistry(_registryAddress);
    }

    function mintOfficialDocument(
        address to,
        string memory uri,
        bool _soulbound,
        ...
        require(
            ...
            "Only verified issuers can mint official documents"
        );
        require(_documentHash != bytes32(0), "Document hash cannot be empty");
        require(
            hashToToken[_documentHash] == 0,
    ...
    function _update(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (doc.isSoulbound) {
                ...
                // Boleh ditransfer CUMA oleh Issuer (Recovery Mechanism jika wallet penerima hilang)
                if (doc.isVerified) {
                    require(
                        msg.sender == doc.issuer,
                        "SBT: Only Issuer can transfer"
                    );
                }
